...
/*
createWorkSpace: Prints LOGO heading and other messages, when program starts, it calls drawboard
 Pre: Empty screen
 Post: Messages gets printed, all 5 boards get drawn
*/
void createWorkSpace(void);

/*
drawBoard: Prints all 5 board on the screen
 Pre: Screen without workspaces, it takes starting Y-X index i.e.startY and startX,
      size of the board to draw i.e sizeY, sizeX, character to draw X and Y boarder i.e. boaderY, boarderX,
      boarder color i.e. colPair and to make boarder bold or not i.e. isBold
 Post: All 5 boards get drawn i.e. game border, message space, command space,
...
/*
displayInfoInInfoSpace: Prints Turtle related information like X,Y co -ordinate and and angle in Info Window
 Pre: Either Info Window will be blank or unupdated
 Post: Updates the Info Window with Turtle's latest values, It takes Turtle struct and use it to display Info
*/

void displayInfoInInfoSpace(Turtle* turtle);

/*
hideInfoFromInfoSpace: Clears the Info window
 Pre: Info Window with Turtle related information
 Post: Clears the Info Space
*/
void hideInfoFromInfoSpace(void);

/*
displayMessageInMessageSpace: This function displays Success or Error messages in Message Space, It calls displayErrorMessage or
        displaySuccessMessage function based on the isSuccess parameter i.e. if isSuccess is 0 -> error or 1-> success
 Pre: It takes isSuccess, error or success message to display based on the code i.e. errorOrSuccessCode and message to displa
        i.e. enteredCommandOutput string
 Post: It displays Error or success messages based on isSuccess's value
*/
void displayMessageInMessageSpace(int isSuccess, int errorOrSuccessCode, char enteredCommandOrOutput[]);

/*
hideMessageFromMessageSpace: This function clears the Message space
 Pre: Message space with unupdated values
 Post: It clears the message space so that new messages can be written after command execution
*/
void hideMessageFromMessageSpace(void);

/*
displayErrorMessage: This function displays the additional error message based on errorCode in Message space
 Pre: It takes errorCode to display error message based on the code plus enteredCommand string to display along with the message
 Post: Error message based on errorCode will be displayed in Message space
*/
void displayErrorMessage(int errorCode,char enteredCommand[]);

/*
displaySuccessMessage: This function displays the additional success message based on successCode in Message space
 Pre: It takes successCode to display success message based on the code plus outputstring to display along with the message
 Post: Success message based on successCode will be displayed in Message space
*/
void displaySuccessMessage(int successCode, char output[]);

/*
hideMessageFromCommandSpace: This function clears the command space
 Pre: Unupdated Command space
 Post: CLeared command space to enter new commands
*/
void hideMessageFromCommandSpace(void);

/*
cleanDrawingSpace: This function clears the drawing space
 Pre: Unupdated Command space
 Post: removed forward, backward, diagonal lines by turtle from fd, bk and setxy command and clean drawing space
*/
...
/*
displayTurtleInDrawingSpace: This function displays the turtle in drawing space
 Pre: No turtle or unupdated turtle in drawing space, it take strct turtle and based on turtle's
    posX, posY, angle co-ordinates, displays the turtle
 Post: Turtle is printed in drawing space
*/
void displayTurtleInDrawingSpace(Turtle* turtle);

/*
hideTurtleFromDrawingSpace: This function hides the turtle in drawing space
 Pre: Printed Turtle on drawing space, this function takes struct turtle and based on the co-ordinates i.e.
    posX, posY, angle, hides the Turtle
 Post: Turtle is cleared in drawing space
*/
void hideTurtleFromDrawingSpace(Turtle* turtle);

/*
drawLines: This function draws the forward and backward lines based on fd and bk command, it also draws diagonal lines if
   Turtle is at 45 or 135 or 225 or 315 degree angle, This function gets called from processOutput function
 Pre: It takes turtle to get xPos and yPos co-ordinate to begin drawing line from that position, char to store
    what position to move i.e. fwdOrbk -> 'f' for forward and 'b' for backward and number of steps to move i.e. stepsToMove
 Post: Lines are drawn in drawing space based on the passed parameters, it also returns an integer i.e. number of steps actually moved
*/
int drawLines(Turtle* turtle,char fwdOrbk, int stepsToMove);

/*
processEnteredCommandInput: This function takes the input string, breaks it to array of the strings, passes it to getOutput function which
 ...
 Pre: It takes input string that was entered by user i.e.enteredInput, total count of the command i.e. count ,
    string to store output i.e. storeOutput, array of string to store all the commands entered by the User i.e. allEnteredInputs
    and struct turtle
 Post: Output gets stored in storeOutput
*/
void processEnteredCommandInput(char* enteredInput,int count, char** storeOutput,char* allEnteredInputs[],Turtle* turtle);


/*
getOutput: This function takes the array of strings i.e. user entered Input from processEnteredCommandInput function
 ...
 Pre: It takes array of string wordsInput i.e. input broken in to words, count of words i.e. counter and turtle
 Post: It returns the outPut string
*/
char* getOutput(char* wordsInput[], int counter, Turtle* turtle);

/*
processOutput: This function takes the user entered input if it has allowed commands and based on the command
   process the output i.e. gets tarhe output, shows the output, call the functions to display the output related
   messages, handles the error condition
 Pre: It takes string to store output i.e.ptrOutput, array of string of words i.e. wordsInput, command number i.e. commandNum
    struct turtle
 Post: It returns the outPut string
*/
...
/*
fixUI: This function fixes the distorted UI of command space and game border, after 'Enter' key is pressed
 Pre: Broken command window and game border because user pressed 'enter'
 Post: fixed the broken UI
*/
void fixUI(void);

/*
fileOperation: This function stores the array of string i.e. user's entered command during the program
   execution in a file
 Pre: It takes array of string of the commands entered by the user during program execution i.e. allEnteredInputs,
    count of total commands i.e. count
 Post: It stores all the commands in a file at assets\logocommand.txt, It returns 'y' if file gets stored properly
*/
