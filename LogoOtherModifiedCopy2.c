...
void displayTurtleInDrawingSpace(Turtle* turtle)
{
    ...
    /* Draw the character at old location where turtle was standing*/
        ...
        case 2:   /* fd */
            {
                ...
                else if(totalWords == 1)
                {
                   ...
                   else /* valid command*/
                   {
                        ...
                        {
                           drawLines(turtle,'f', temp); /* this function will draw the lines*/
                        }
        ...
        case 4:   /* bk */
            {
                 ...
                 else if(totalWords == 1) /* 2 words input */
                {
                   ...
                   else /* valid command*/
                   {
                        ...
                        {
                          drawLines(turtle,'b', temp); /* call this function to draw backward line*/

                        }
                        else /* if negative number input */
                        {
                            temp = temp * (-1);
                            drawLines(turtle,'f', temp); /* draw forward lines*/
                        }
               ...
               else if(totalWords == 1) /* 2 words input */
               {
                   ...
                   else /* valid input with second word as a number*/
                   {
                       ...
                       if((enteredAngle == 45) || (enteredAngle == 90)) /*if entered input is 45 or 90*/
                       {
                           ...
                           hideTurtleFromDrawingSpace(turtle);
                            cacheCharAtTurtleLoc(turtle->posY, turtle->posX, tempAngle, turtle);
...
/*
This function caches the lines in character so that when the turtle moves next time, the original lines
can be restored
It takes new position of turtle x, y and angle as input i.e. newPosX, newPosY and newPosAngle and address of turtle
It doesn't return anything
*/
void cacheCharAtTurtleLoc(int newPosY, int newPosX, int newPosAngle, Turtle* turtle)
{
    turtle->oldLocY = turtle->posY;
    turtle->oldLocX = turtle->posX;
    turtle->oldAngle = turtle->angle;
    int tempOldLocY = 0, tempOldLocX = 0;
    tempOldLocY = newPosY;
    tempOldLocX = newPosX;

    mvprintw(5,10,"%c",turtle->preCharAtBody);
    mvprintw(6,10,"%c",turtle->preCharAtHead);
    mvprintw(7,10,"%d",newPosAngle);
    mvprintw(8,10,"%d",newPosAngle);
    mvprintw(9,10,"%d",turtle->oldLocY);
    mvprintw(10,10,"%d",turtle->oldLocX);
